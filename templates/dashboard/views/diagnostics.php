...
<!-- Script de Sondeo en Tiempo Real para el Visor de Logs -->
<script>
(function() {
    'use strict';
    ...
    // Calcular el último id inicial en el DOM
    ...
    if (btnPause) {
        btnPause.addEventListener('click', function() {
            isPaused = !isPaused;
            if (isPaused) {
                txtPause.textContent = 'Reanudar';
                badge.style.background = 'var(--warning-soft)';
                badge.style.color = 'var(--warning)';
                badge.innerHTML = '<span>PAUSADO</span>';
            } else {
                txtPause.textContent = 'Pausar';
    ...
    if (btnScroll) {
        btnScroll.addEventListener('click', function() {
            ...
            txtScroll.textContent = autoScroll ? 'ON' : 'OFF';
            if (autoScroll) {
                consoleBox.scrollTop = consoleBox.scrollHeight;
            }
        });
    ...
    if (selSource) selSource.addEventListener('change', applyLocalFilters);
    if (selLevel) selLevel.addEventListener('change', applyLocalFilters);
    if (inputSearch) inputSearch.addEventListener('input', applyLocalFilters);

    // Sondeo de nuevos logs cada 2 segundos
    var logPollTimer = setInterval(function() {
        if (isPaused) return;
            ...
            .then(function(data) {
                if (data.logs && data.logs.length > 0) {
                    data.logs.forEach(function(l) {
                        ...
                        div.innerHTML = [
                            '<span class="log-time">[' + (l.timeOnly || l.timestamp) + ']</span>',
                    ...
                    // Limitar a máximo 200 líneas en el DOM
